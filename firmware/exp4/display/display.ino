...
void receiveFrame(const struct can_frame &frame) {
  if (frame.can_id == 0x110) {
    ...
    if (frame.can_dlc != 4 || frame.data[0] != 0xB4 ||
        frame.data[2] < 30 || frame.data[2] > 150 || frame.data[3] > 1 ||
        (frame.data[3] == 0 && frame.data[2] != 90)) {
      statusLink.steps = 0;
      return;
    }
    track(statusLink, frame.data[1]);
    appliedAngle = frame.data[2];
    actuatorActive = frame.data[3] == 1;
  }
}
...
void refreshDisplay() {
  ...
  if (!healthy(statusLink))
    ...
    snprintf(row1, sizeof(row1), "UYG:%3u %-7s ", static_cast<unsigned>(appliedAngle),
             actuatorActive ? "AKTIF" : "BEKLE");
  if (lcdReady) {
    lcd.setCursor(0, 0); lcd.print(row0);
    lcd.setCursor(0, 1); lcd.print(row1);
  }
  Serial.print(row0); Serial.print(F(" | ")); Serial.println(row1);
}
...
void setup() {
  ...
  Serial.println(F("EXP04 NANO DISPLAY"));
}
